...
def put_text_on_image(max_row, _max_len_in_row, text, size):
    new_str = '\n'.join(line.strip() for line in re.findall(r'.{1,'+str(_max_len_in_row)+'}(?:\s+|$)', text))
    str_lines = new_str.split("\n")
    if(len(str_lines) > max_row):
        str_lines = str_lines[0:max_row-1]
    step = 0
    s = " "
    for i in range(_max_len_in_row - len(str_lines[-1])): 
        s += " "
    str_lines[-1] = str_lines[-1] + s

    if size == "small":
        font = ImageFont.truetype("/home/datis/development/datisSpeed/src/data/BNazanin.ttf", 12)
        image = Image.new("RGB", [_max_len_in_row*5, max_row*25], color="#000000")
        draw = ImageDraw.Draw(image)
        for line in str_lines:
            reshaped_text = default_reshaper.reshape(line)
            bidi_text = get_display(reshaped_text)
            draw.text((0, step), bidi_text, font=font, fill=(255, 255, 255, 255))
            step += 25
    else:
        font = ImageFont.truetype("/home/datis/development/datisSpeed/src/data/BNazanin.ttf", 25)
        image = Image.new("RGB", [_max_len_in_row*4, max_row*30], color="#000000")
        draw = ImageDraw.Draw(image)
        for line in str_lines:
            reshaped_text = default_reshaper.reshape(line)
            bidi_text = get_display(reshaped_text)
